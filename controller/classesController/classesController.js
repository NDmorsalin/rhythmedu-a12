...
const {
  StudentsSelectedClass
} = require("../../Models/Students/StudentsSelectedClass");
const {
  EnrolledClasses
} = require("../../Models/Students/enrolledClasses");
const {
  User
...
const getAllClasses = async (req, res, next) => {
  ...
  try {
    if (studentId) {
      ...
      const selectedClassOfThisStudent = await StudentsSelectedClass.find({
        studentId
      }).toArray()
      const paidClasses = await EnrolledClasses.find({
        studentId
      }).toArray()
...
const adminClasses = async (req, res, next)=> {
...
const getInstructorClasses = async (req, res, next) => {
  ...
  // console.log(email);
  try {
    const instructorClasses = await Classes.find({
...
const updateInstructorClasses = async (req, res, next) => {
  ...
  try {
    ...
    const updateInfo = await Classes.updateOne( {
...
const deleteInstructorClasses = async (req, res, next) => {
  ...
  // console.log({ classId });
  try {
    const deleteInfo = await Classes.deleteOne( {
      _id: new ObjectId(classId)
    })
...
const singleInstructorClasses = async (req, res, next) => {
  ...
  try {
    ...
    const instructorClasses = await Classes.find({
      instructorEmail: instructor.email, status: "approved"
    }).toArray()
    ...
    if (studentId) {
      ...
      res.status(200).json( {
        instructor,
        instructorClasses: instructorClassesWithStudentId
      })
    } else {
      res.status(200).json( {
...
const getPopularClasses = async (req, res, next) => {
  try {
    const popularClasses = await Classes.find({
      status: "approved"
    }).sort({
      enrolledStudents: -1
    }).limit(6).toArray()
